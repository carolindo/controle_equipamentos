module SendDocHelper
  ...
  def write_doc(nome_arquivo, xml, xml_start_path, report, filename, output_type = 'pdf')
    ...
    cache_hack
    pdf = Document.generate_report(xml, report, jasper_type, xml_start_path)
    File.open(nome_arquivo, 'wb') {|f| f << pdf }
  end
end
